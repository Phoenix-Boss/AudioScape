...
import { extractionKeys } from '../extraction/useExtractionChamber';
...
class SelfHealing {
  ...
  initialize(optInCommunityIntelligence: boolean = false): void {
    ...
    console.log('[Mavin SelfHealing] System initialized (opt-in:', this.isOptedIn, ')');
  }
  ...
  async checkForUpdates(): Promise<{
    ...
    try {
      ...
      if (update.isAvailable) {
        ...
        const validatedManifest = OTAUpdateManifestSchema.parse(update.manifest);
        
        console.log('[Mavin SelfHealing] OTA update available:', validatedManifest.metadata.version);
  ...
  async applyUpdate(): Promise<{
    ...
    try {
      ...
      try {
        ...
        console.log('[Mavin SelfHealing] OTA update failed. Rolled back to previous version.');
  ...
  private scheduleOTAChecks(): void {
    ...
    this.monitoringInterval = setInterval(async () => {
      ...
      if (updateCheck.available) {
        console.log('[Mavin SelfHealing] Non-critical OTA update available. Will apply on next app restart.');
      }
  ...
  async syncCommunityCache(): Promise<{
    ...
    if (!this.isOptedIn) {
      console.log('[Mavin SelfHealing] Community sync skipped (opt-in required)');
    ...
    try {
      ...
      const decipherFunctions = z.array(CommunityDecipherSchema).parse(data.decipherFunctions || []);
      const healthyKeys = z.array(KeyHealthReportSchema).parse(data.healthyKeys || []);
      ...
      console.log(`[Mavin SelfHealing] Synced ${decipherFunctions.length} decipher functions, ${healthyKeys.length} healthy keys`);
  ...
  private startHealthMonitoring(): void {
    ...
    console.log('[Mavin SelfHealing] Health monitoring started (every 5 minutes)');
  }
  ...
  private async monitorHealth(): Promise<void> {
    try {
      ...
      if (recoveryActions.length > 0) {
        console.log(`[Mavin SelfHealing] Triggering ${recoveryActions.length} recovery actions`);
      ...
      // Log health status
      console.log(`[Mavin SelfHealing] Health check OK | Success: ${metrics.extractionSuccessRate.toFixed(1)}% | Cache: ${metrics.cacheHitRate.toFixed(1)}%`);
  ...
  private async gatherHealthMetrics(): Promise<HealthMetrics> {
    ...
    const successfulQueries = extractionQueries.filter(([, data]) => 
      data && !('error' in data)
    ).length;
      ...
      try {
        switch (action) {
          case 'INVALIDATE_EXTRACTION_CACHE':
            ...
            console.log('[Mavin SelfHealing] Recovery: Invalidated extraction cache');
            break;
            
          case 'CLEANUP_CACHE':
            // Trigger MavinCache cleanup
            // Note: In production, MavinCache would have cleanup method
            console.log('[Mavin SelfHealing] Recovery: Triggered cache cleanup');
            break;
            
          case 'ROTATE_API_KEYS':
            // In production: Signal KeyManager to rotate keys
            console.log('[Mavin SelfHealing] Recovery: Triggered API key rotation');
            break;
            
          case 'ACTIVATE_FALLBACK_SOURCES':
            // In production: Update source priority configuration
            console.log('[Mavin SelfHealing] Recovery: Activated fallback sources');
            break;
            
          default:
            console.warn(`[Mavin SelfHealing] Unknown recovery action: ${action}`);
        }
      } catch (error) {
        const mavinError = errorFromUnknown(error);
        logError(mavinError, 'error');
  ...
  getHealthStatus(): {
    ...
    return {
      isHealthy: this.lastHealthReport 
        ? this.lastHealthReport.extractionSuccessRate >= this.thresholds.extractionSuccessRate
        : true,
...
export type { CommunityDecipher, KeyHealthReport, HealthMetrics, OTAUpdateManifest };