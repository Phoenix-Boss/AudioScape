...
import { useExtractionChamber, ExtractionResult } from '../extraction/useExtractionChamber';
...
const EngineStateSchema = z.object({
  status: z.enum(['idle', 'loading', 'playing', 'paused', 'buffering', 'error']),
  ...
  premiumStatus: z.enum(['free', 'grace_period', 'trial_active', 'premium', 'expired']),
  lastError: z.object({
    code: z.string(),
    message: z.string(),
    timestamp: z.number(),
...
export const useMavinEngine = (options: UseMavinEngineOptions = {}) => {
  ...
  const extraction = useExtractionChamber(
  ...
  const dispatch = React.useCallback((action: EngineAction) => {
    ...
    switch (action.type) {
      ...
      case 'TOGGLE_REPEAT':
        ...
        break;
      default:
        console.warn('[Mavin Engine] Unknown action type:', (action as any).type);
    }
  }, [engineQuery.data]);

  // Play action handler
  const handlePlayAction = React.useCallback(async (action: z.infer<typeof PlayActionSchema>) => {
    ...
    // Trigger extraction (will call onSuccess when ready)
    // Extraction hook already handles the rest via onSuccess callback
  }, [engineQuery.data]);

  // Extraction success handler
  const handleExtractionSuccess = React.useCallback(async (result: ExtractionResult, track: NonNullable<EngineState['currentTrack']>) => {
  ...
  const handleExtractionError = React.useCallback(async (error: ExtractionError) => {
    await updateEngineState(draft => {
      draft.status = 'error';
      draft.lastError = {
        code: error.code,
  ...
  const handleNextAction = React.useCallback(async () => {
    ...
    // Handle repeat mode
    if (engineQuery.data.isRepeat && nextIndex >= engineQuery.data.playQueue.length) {
      nextIndex = 0;
    }
  ...
  // Seek handler
  const handleSeekAction = React.useCallback(async (position: number) => {
  ...
  // Set queue handler
  const handleSetQueueAction = React.useCallback(async (
    tracks: Array<{ id: string; videoId: string; title: string; artist: string }>,
    startIndex?: number
  ) => {
    // Validate tracks
    const parsedTracks = z.array(z.object({
      id: z.string().min(1),
      videoId: z.string().min(10),
      title: z.string().min(1),
      artist: z.string().min(1),
  ...
  const updateEngineState = React.useCallback(async (updater: (draft: EngineState) => void) => {
    const currentState = queryClient.getQueryData<EngineState>(engineQueryKey);
  ...
  const trackPlayEvent = React.useCallback((
    ...
    console.log('[Mavin Analytics]', { trackId, videoId, event, ...metadata });
  ...
  React.useEffect(() => {
    ...
    const subscription = AppState.addEventListener('change', (nextState) => {
      ...
      if (isBackground && premiumStatus.status !== 'premium') {
  ...
  return {
    ...
    setQueue: (tracks: Array<{ id: string; videoId: string; title: string; artist: string }>, startIndex?: number) =>
    ...
    isPremium: premiumStatus.status === 'premium' || premiumStatus.status === 'trial_active',
...
const getDeviceFingerprint = async (): Promise<string> => {
  ...
  // Mock implementation for now
  return 'mock_device_fingerprint_' + Math.random().toString(36).substr(2, 9);
};
...
export interface ExtractionError {
  code: 'NO_SOURCES_AVAILABLE' | 'ALL_SOURCES_FAILED' | 'NETWORK_ERROR' | 'INVALID_VIDEO_ID' | 'CONTENT_UNAVAILABLE' | 'RATE_LIMITED' | 'DECRYPTION_FAILED' | 'PLAYBACK_FAILED';
  message: string;
