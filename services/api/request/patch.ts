// src/services/api/request/patch.ts
...
export default async function patchRequest<T = any>({
  url,
  data = {},
  params = {},
  isWithSelfId = false,
  ...
  onError,
  onComplete,
}: ApiPatchParams): Promise<T | undefined> {
  ...
  try {
    ...
    console.log(`✅ PATCH ${url} (${responseTime}ms)`);
